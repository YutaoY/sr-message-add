  ...
  <div class="character-select" v-show="input.select">
    <div class="box" @click.stop="">
      <div class="list">
        ...
        <div class="item">
          <div class="character" title="开拓者" @click="handelcharacterClick('开拓者')">
            <div class="avatar">
              <img :src="user[setting.type].card" :alt="getName('开拓者')" draggable="false" />
              <div class="name">{{ getName('开拓者') }}</div>
            </div>
            <div class="info"></div>
          </div>
          <div class="character" v-for="(item, key) in character.game" :key="`avatar-${key}`"
            :title="`${getName(item.name)}`" @click="handelcharacterClick(String(key))">
            <div class="avatar">
        ...
        <div class="item">
          <div class="character" v-for="(item, key) in character.custom" :key="`avatar-${key}`"
            @click="handelcharacterClick(String(key))">
            <div class="avatar" style="height: 'var(--character-item-width)'">
              <img :src="item.avatar" :alt="item.name" :title="item.name" draggable="false" />
            </div>
          ...
          <div class="add" @click="addCustom">
            <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="150" height="150">
              <path
...
const handelcharacterClick = (key: string) => {
  if (input.index) {
    ...
    const messageData = message.list[input.index[0]].list[input.index[1]]
    input.index = undefined
    if (key === '开拓者') {
      if (messageData.chekbox === undefined) {
        messageData.chekbox = 0
      }
      if (messageData.option) {
        messageData.text = messageData.option[(messageData.chekbox) as number]
      }
    } else {
      if (messageData.chekbox !== undefined) {
        messageData.chekbox = undefined
      }
      if (messageData.option) {
        messageData.text = '[对话选项卡]'
      }
    }
  } else {
    input.character = key
